...
public class Solution {
    ...
    public static Class getExpectedClass() {
        ...
        for (Class collectionsClass : collectionsClasses) {
            if (List.class.isAssignableFrom(collectionsClass)
                    && Modifier.isPrivate(collectionsClass.getModifiers())
                    && Modifier.isStatic(collectionsClass.getModifiers())){

                try {
                    Constructor constructor = collectionsClass.getDeclaredConstructor();
                    constructor.setAccessible(true);
                    List list = (List)constructor.newInstance();
                    try{
                        list.get(0);
                    } catch (IndexOutOfBoundsException e){
                        return collectionsClass;
                    }
                } catch (Exception e) {
                }
            }
