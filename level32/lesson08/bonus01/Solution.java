...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class<T> returnClassType, Class<?>... interfaces) {

        if (!Item.class.isAssignableFrom(returnClassType)) return null;

        ClassLoader classLoader = returnClassType.getClassLoader();
        Class<?>[] classes = new Class<?>[interfaces.length+1];
        System.arraycopy(interfaces, 0, classes, 0, interfaces.length);
        classes[classes.length-1] = returnClassType;

        Object proxy = Proxy.newProxyInstance(classLoader, classes, new ItemInvocationHandler());
